        ...
        <div class="xl:col-span-7 xl:sticky xl:top-4 space-y-4">
            ...
            <div class="overflow-x-auto p-4 bg-gray-200/70 rounded-2xl border border-gray-300/80 shadow-inner">
                ...
                <div id="invoice-print-area" 
                    ...
                    <!-- KOP SURAT RESMI CV ASIANINDO (PROPORSI RAMPING & PADAT SESUAI MEDIA ACUAN) -->
                    <div class="flex items-center justify-between gap-3 pb-1">
                        <!-- Left Logo (Proporsi Pas Sesuai Contoh media_1789114031143) -->
                        <div class="flex-shrink-0">
                            <img src="../images/logo_invoice_official.png" alt="CV. Asianindo" class="w-32 h-auto object-contain">
                        </div>

                        <!-- Right Company Details (CENTERED & RAMPING ATAS-BAWAH) -->
                        <div class="flex-1 text-center pl-2">
                            <h1 class="text-[24px] font-bold text-[#1e3a8a] uppercase tracking-wide leading-none mb-1">CV. ASIANINDO</h1>
                            <p class="text-[12px] font-bold text-black leading-tight mb-0.5">
                            ...
                            <p class="text-[11px] font-bold text-black leading-tight mt-0.5">
                    ...
                    <div class="border-b-[2.5px] border-black my-2"></div>

                    <!-- Document Title -->
                    <div class="text-center my-2">
                        <h2 class="text-[18px] font-bold uppercase tracking-[0.25em] text-black" x-text="invoiceForm.docType"></h2>
                    </div>

                    <!-- Kepada Yth & Tanggal Header (Times New Roman Bold Persis Media Acuan) -->
                    <div class="flex justify-between items-start text-[12px] leading-tight mb-2.5 font-serif-invoice">
                    ...
                    <!-- TABEL UTAMA DOKUMEN (Border Hitam Solid 2px Persis Dokumen Acuan media_1789113986907) -->
                    <div class="border-2 border-black">
                        <table class="w-full border-collapse text-[12px] leading-tight">
                            <thead>
                                <tr class="border-b-2 border-black text-center font-bold text-black uppercase bg-white text-[12.5px]">
                                    <th class="py-2 px-2 border-r border-black w-12 text-center font-bold">NO</th>
                                    <th class="py-2 px-3 border-r border-black text-center font-bold" x-text="invoiceForm.tableHeaderName || 'KETERANGAN'"></th>
                                    <th class="py-2 px-3 border-r border-black text-center font-bold" x-show="invoiceForm.hasSpecs">SPESIFIKASI</th>
                                    <th class="py-2 px-2 border-r border-black w-12 text-center font-bold" x-text="invoiceForm.tableHeaderQty || 'QTY'"></th>
                                    <th class="py-2 px-3 text-center w-40 font-bold">TOTAL</th>
                                </tr>
                            </thead>
                            <tbody>
                                <template x-for="(item, idx) in invoiceForm.items" :key="idx">
                                    <tr class="border-b border-black align-top">
                                        <td class="p-2.5 text-center font-bold border-r border-black text-[12px] text-black" x-text="(idx + 1) + '.'"></td>
                                        <td class="p-2.5 border-r border-black font-bold text-black text-[12px]" x-text="item.name"></td>
                                        <td class="p-2.5 border-r border-black" x-show="invoiceForm.hasSpecs">
                                            <!-- Font Spesifikasi Sama dengan Font Keterangan & Ditebalkan Persis Contoh media_1789113986907 -->
                                            <div class="space-y-1 text-[12px] leading-snug text-black font-bold">
                                        ...
                                        <td class="p-2.5 text-center font-bold border-r border-black text-[12px] text-black" x-text="item.qty || 1"></td>
                                        <td class="p-2.5 text-right font-bold text-black whitespace-nowrap text-[12px]" x-text="formatRupiahInvoice(item.price * item.qty)"></td>
                                    </tr>
                                </template>
                                ...
                                <tr :class="invoiceForm.yellowTotal ? 'bg-[#FFE500]' : 'bg-white'" 
                                    ...
                                    class="font-bold border-b border-black text-[12.5px]">
                                ...
                                <template x-if="invoiceForm.paymentTermMode === 'dp_single'">
                                    <tr class="font-bold border-b border-black text-[12px]">
                    ...
                    <!-- 1. KETERANGAN NOTES (DI KIRI DI BAWAH TABEL DENGAN FONT BOLD IDENTIK) -->
                    <div class="mt-3.5 pt-1 text-[12px] leading-relaxed text-black max-w-xl">
                        <div class="font-bold text-[12.5px]">Keterangan:</div>
                        <div class="space-y-1.5 mt-1 pl-1">
                            <!-- Rekening Item -->
                            <div class="flex items-start gap-1.5">
                                <span class="font-bold select-none">•</span>
                                <span class="font-bold" x-show="invoiceForm.bankAccountType === 'iman'">Rekening BCA 6670747997 a/n Iman Anjani Buchory</span>
                                <span class="font-bold" x-show="invoiceForm.bankAccountType === 'asianindo'">Rekening BCA 0113582348 a/n CV.Asianindo.</span>
                                <span class="font-bold" x-show="invoiceForm.bankAccountType === 'custom'" x-text="invoiceForm.bankAccountCustom"></span>
                            </div>

                            <!-- Ongkir Item -->
                            <div class="flex items-start gap-1.5" x-show="invoiceForm.shippingOption !== 'none'">
                                <span class="font-bold select-none">•</span>
                                <span class="font-bold" x-show="invoiceForm.shippingOption === 'exclude'">Harga belum termasuk ongkos kirim ke lokasi</span>
                                <span class="font-bold" x-show="invoiceForm.shippingOption === 'free'">Free Ongkos Kirim ke Lokasi</span>
                                <span class="font-bold" x-show="invoiceForm.shippingOption === 'include'">Sudah termasuk ongkos kirim</span>
                                <span class="font-bold" x-show="invoiceForm.shippingOption === 'custom'" x-text="invoiceForm.shippingCustom"></span>
                            </div>
                            ...
                            <div x-show="invoiceForm.showPaymentScheme">
                                <div class="flex items-start gap-1.5">
                                    <span class="font-bold select-none">•</span>
                                    <span class="font-bold">Skema pembayaran :</span>
                                </div>
                    ...
                    <!-- 2. TANDA TANGAN DIREKTUR (DI BAWAH KETERANGAN, SISI KANAN DENGAN STEMPEL PROPORSIONAL BESAR PERSIS CONTOH media_1789114102479) -->
                    <div class="flex justify-end mt-2 pt-1" x-show="invoiceForm.showSignature">
                        <div class="text-center w-52 text-[12px] leading-tight">
                            <div class="font-bold text-black text-[12px]">Hormat Kami,</div>
                            <div class="font-bold text-black uppercase tracking-wide text-[13px] mt-0.5">CV ASIANINDO</div>
                            
                            <!-- Authentic Stamped Signature (Proporsi Pas Mengisi Tengah media_1789114102479) -->
                            <div class="my-1 flex justify-center items-center h-20 relative">
                                <img src="../images/signature_authentic_stamp.png" class="w-44 h-auto object-contain pointer-events-none select-none">
                            </div>
                            ...
                            <div class="font-bold text-black uppercase text-[11px] mt-1" x-text="invoiceForm.signerTitle || 'DIREKTUR'"></div>
                        </div>
                    </div>
